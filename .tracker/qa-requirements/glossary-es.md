# Translation glossary — Spanish (es)
...
Native name: **Español**. Sheet tab and URL prefix: `es` / `/es`.
Connector code: `es`.
...
Facts about Spanish that tier 1 already enforces deterministically. They are written
...
| Text expansion vs English | 1.25x | `measureExpansion` divides the observed ratio by this before applying `qa.wordRatio`, so Spanish running 1.25x longer is expected and not a finding |
...
**Empty because nothing has been translated into Spanish yet.** Every row added here is
