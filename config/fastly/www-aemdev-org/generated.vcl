...
pragma optional_param customer_id "6WBvFEG91w4deOfZw2XAWY";
C!
W!
# Backends

backend F_Helix_5_DA_Origin {
    .always_use_host_header = true;
    .between_bytes_timeout = 10s;
    .connect_timeout = 1s;
    .dynamic = true;
    .first_byte_timeout = 15s;
    .host = "main--aemdev--aemgdc.aem.live";
    .host_header = "main--aemdev--aemgdc.aem.live";
    .max_connections = 200;
    .port = "443";
    .share_key = "tTSyFJgoN3lZqBOA0n3Rd7";

    .ssl = true;
    .ssl_cert_hostname = "main--aemdev--aemgdc.aem.live";
    .ssl_check_cert = always;
    .ssl_sni_hostname = "main--aemdev--aemgdc.aem.live";

    .probe = {
        .dummy = true;
        .initial = 5;
        .request = "HEAD / HTTP/1.1"  "Host: main--aemdev--aemgdc.aem.live" "Connection: close";
        .threshold = 1;
        .timeout = 2s;
        .window = 5;
      }
...
# Snippet strip query string on vcl_rcv : 100
...
# Snippet strip query string on vcl_rcv end


  # default conditions
  set req.backend = F_Helix_5_DA_Origin;
  ...
  {
    if (req.backend == F_Helix_5_DA_Origin && var.fastly_req_do_shield) {
      set req.backend = fastly.try_select_shield(ssl_shield_iad_va_us, F_Helix_5_DA_Origin);
    }
  }
...
# Snippet strip age header : 100
unset resp.http.Age;
# Snippet strip age header end
...
# Snippet set byocdn var for pass : 100
set bereq.http.X-BYO-CDN-Type = "fastly";
set bereq.http.X-Push-Invalidation = "enabled";
# Snippet set byocdn var for pass end
...
#--FASTLY LOG BEGIN
  ...
  # s3 Adobe_llmo
  log {"syslog "} req.service_id {" Adobe-llmo :: "} {"{     "timestamp": ""} strftime({"%Y-%m-%dT%H:%M:%S%z"}, time.start) {"",     "host": ""} if(req.http.Fastly-Orig-Host, req.http.Fastly-Orig-Host, req.http.Host) {"",     "url": ""} json.escape(req.url) {"",     "request_method": ""} json.escape(req.method) {"",     "request_referer": ""} json.escape(req.http.referer) {"",     "request_user_agent": ""} json.escape(req.http.User-Agent) {"",     "response_status": "} resp.status {",     "response_content_type": ""} json.escape(resp.http.Content-Type) {"",     "client_country_code": ""} client.geo.country_name {"",     "time_to_first_byte": ""} time.to_first_byte {"" }"};
