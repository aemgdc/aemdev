# Translation glossary — Chinese (Simplified) (zh-cn)
...
Native name: **简体中文**. Sheet tab and URL prefix: `zh-cn` / `/zh-cn`.
Connector code: `zh-CN` — note the case difference from our own id; see scripts/tracker/locales.js.
...
| English | Required Chinese (Simplified) | Note |
| --- | --- | --- |
| block | | |
| section | | |
| author (the person) | | |
...
Facts about Chinese (Simplified) that tier 1 already enforces deterministically. They are written
...
| Text expansion vs English | 0.5x | `measureExpansion` divides the observed ratio by this before applying `qa.wordRatio`, so Chinese (Simplified) running 0.5x longer is expected and not a finding |
...
**Empty because nothing has been translated into Chinese (Simplified) yet.** Every row added here is
...
make Chinese (Simplified) shorter).
