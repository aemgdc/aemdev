# Translation glossary — French (fr)
...
Native name: **Français**. Sheet tab and URL prefix: `fr` / `/fr`.
Connector code: `fr`.
...
Facts about French that tier 1 already enforces deterministically. They are written
...
| --- | --- | --- |
| Thousands separator | a narrow no-break space (U+202F / U+00A0) | `extractNumbers` compares figures as VALUES, so `176,000` → `176 000` passes and `1.5` → `15` does not. |
| Decimal separator | `,` | same |
| Quotation marks | « … » with spaces | `quoteStyleCheck`, as a NOTE only |
| Text expansion vs English | 1.25x | `measureExpansion` divides the observed ratio by this before applying `qa.wordRatio`, so French running 1.25x longer is expected and not a finding |
...
**Empty because nothing has been translated into French yet.** Every row added here is
