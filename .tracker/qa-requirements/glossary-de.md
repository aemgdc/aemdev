...
Native name: **Deutsch**. Sheet tab and URL prefix: `de` / `/de`.
Connector code: `de`.
...
Facts about German that tier 1 already enforces deterministically. They are written
...
| Text expansion vs English | 1.3x | `measureExpansion` divides the observed ratio by this before applying `qa.wordRatio`, so German running 1.3x longer is expected and not a finding |
...
**Empty because nothing has been translated into German yet.** Every row added here is
