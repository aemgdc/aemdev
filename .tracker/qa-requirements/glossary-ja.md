...
Native name: **日本語**. Sheet tab and URL prefix: `ja` / `/ja`.
Connector code: `ja`.
...
| English | Required Japanese | Note |
| --- | --- | --- |
| block | | |
| section | | |
| author (the person) | | |
...
Facts about Japanese that tier 1 already enforces deterministically. They are written
...
| Text expansion vs English | 0.6x | `measureExpansion` divides the observed ratio by this before applying `qa.wordRatio`, so Japanese running 0.6x longer is expected and not a finding |
...
**Empty because nothing has been translated into Japanese yet.** Every row added here is
