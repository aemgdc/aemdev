# Translation glossary — Portuguese (pt)
...
Native name: **Português**. Sheet tab and URL prefix: `pt` / `/pt`.
Connector code: `pt`.
...
| English | Required Portuguese | Note |
| --- | --- | --- |
| block | | |
| section | | |
| author (the person) | | |
...
Facts about Portuguese that tier 1 already enforces deterministically. They are written
...
| Quotation marks | «…» or “…” | `quoteStyleCheck`, as a NOTE only |
| Text expansion vs English | 1.25x | `measureExpansion` divides the observed ratio by this before applying `qa.wordRatio`, so Portuguese running 1.25x longer is expected and not a finding |
...
**Empty because nothing has been translated into Portuguese yet.** Every row added here is
