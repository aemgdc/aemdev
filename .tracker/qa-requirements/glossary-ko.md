# Translation glossary — Korean (ko)
...
Native name: **한국어**. Sheet tab and URL prefix: `ko` / `/ko`.
Connector code: `ko`.
...
Facts about Korean that tier 1 already enforces deterministically. They are written
...
| Text expansion vs English | 0.7x | `measureExpansion` divides the observed ratio by this before applying `qa.wordRatio`, so Korean running 0.7x longer is expected and not a finding |
...
**Empty because nothing has been translated into Korean yet.** Every row added here is
